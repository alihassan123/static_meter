...
<?php
	...
	$packets->inst_1 = array(0x4d,0x54,0x4c,0x53,0x00,0x00,0x00,0xbb,0x0b,0x01,0x01,0x01,0x52,0x01,0x04,0x00,0x00,0x77,0xb5,0x19,0x25,0x00,0x00,0x01,0x00);
	$packets->inst_2 = array(0x4d,0x54,0x4c,0x53,0x00,0x00,0x00,0xbb,0x0b,0x01,0x01,0x01,0x52,0x01,0x04,0x0c,0x00,0x77,0xb5,0x2d,0xb2,0x00,0x00,0x01,0x00);
	$packets->cum_1 = array(0x4d ,0x54 ,0x4c ,0x53 ,0x00 ,0x00 ,0x00 ,0xbb ,0x0b ,0x01 ,0x01 ,0x01 ,0x52 ,0x01 ,0x04 ,0x00 ,0x00 ,0x77 ,0xb5 ,0x19 ,0x25 ,0x00 ,0x00 ,0x01 ,0x00);
	$packets->cum_2 = array(0x4d ,0x54 ,0x4c ,0x53 ,0x00 ,0x00 ,0x00 ,0xbb ,0x0b ,0x02 ,0x01 ,0x01 ,0x52 ,0x02 ,0x01 ,0x00 ,0x00 ,0x77 ,0xb5 ,0x59 ,0x7b ,0x00 ,0x00 ,0x01 ,0x00);
	$packets->cum_3 = array(0x4d ,0x54 ,0x4c ,0x53 ,0x00 ,0x00 ,0x00 ,0xbb ,0x0b ,0x02 ,0x01 ,0x01 ,0x52 ,0x02 ,0x08 ,0x00 ,0x00 ,0x77 ,0xb5 ,0x3d ,0x2a ,0x00 ,0x00 ,0x01 ,0x00);
	$packets->cum_4 = array(0x4d ,0x54 ,0x4c ,0x53 ,0x00 ,0x00 ,0x00 ,0xbb ,0x0b ,0x02 ,0x01 ,0x01 ,0x52 ,0x02 ,0x06 ,0x00 ,0x00 ,0x77 ,0xb5 ,0x85 ,0x4b ,0x00 ,0x00 ,0x01 ,0x00);
	...
	function get_data_from_meter( $fd , $packet , $delay = 1 ) 
	{
		...
		$w = pack_array( 'C*' , $packet );
		echo '<br><br><br>REQUEST ' .strlen($w) .'<br>' .hex_str($w) .' <br> ';
		dio_write( $fd  , $w);
		// echo(date("h:i:s A", time())); 
		sleep($delay);// echo "<br>DELAY<br>";
		// echo(date("h:i:s A", time())); 
		$r = dio_read( $fd );
		echo '<br>RESPONSE ' .strlen($r)  .'<br>' .hex_str($r) .' <br>';
		return $r;
	}
	...
	function hex_str($hex)
	{
	    ...
	    for ($i=0; $i < strlen($hex); $i+=1){
	        $string .= sprintf('%02X ',ord($hex[$i]));
	    }
	    return $string;
	}

	// get_data_from_meter( $fd , $packets->inst_1 , 2 );
	// get_data_from_meter( $fd , $packets->inst_2 , 2 );

	get_data_from_meter( $fd , $packets->cum_1 , 2 );
	get_data_from_meter( $fd , $packets->cum_2 , 2 );
	get_data_from_meter( $fd , $packets->cum_3 , 2 );
	get_data_from_meter( $fd , $packets->cum_4 , 2 );
